...
func _connect_signals() -> void:
	...
	# NPCEventSystem events (adjust path if needed)
...
func _refresh() -> void:
	...
	# --- capture ---
	var prev := _last_selection if _selected_meta.is_empty() else _selected_meta
	prev = {} if prev.is_empty() else prev.duplicate(true)

	# --- rebuild Pending ---
	tree_pending.clear()
	_set_cols(tree_pending, ["JobID","Template","Prio","Target","Pos"])
	var root_p = tree_pending.create_item()
	for j: JobInstance in NPCJobBoard.get_pending_jobs():
		var it = tree_pending.create_item(root_p)
	...
	tree_active.clear()
	_set_cols(tree_active, ["JobID","Template","Prio","Status","Agent","Target","Bindings","Pos/Dist"])
	var root_a = tree_active.create_item()
	for j: JobInstance in NPCJobBoard.get_active_jobs():
		var it = tree_active.create_item(root_a)
		_fill_job_row_active(it, j)

	# --- rebuild Agents ---
	tree_agents.clear()
	_set_cols(tree_agents, ["AgentID","State","Pos","Notes"])
	var root_ag = tree_agents.create_item()
	for ag: WorkerAgent in NPCJobBoard.get_all_agents():
		var it = tree_agents.create_item(root_ag)
...
func _select_job_in_tree(tree: Tree, job_id: int) -> bool:
	return _select_by_predicate(tree, func(item: TreeItem):
		var m:Dictionary = item.get_metadata(0)
		return m is Dictionary and m.get("type") == "job" and int(m.get("job_id", -1)) == job_id
	)

func _select_agent_in_tree(tree: Tree, agent_id: int) -> bool:
	return _select_by_predicate(tree, func(item: TreeItem):
		var m:Dictionary = item.get_metadata(0)
		return m is Dictionary and m.get("type") == "agent" and int(m.get("agent_id", -1)) == agent_id
	)
...
func _select_by_predicate(tree: Tree, pred: Callable) -> bool:
	...
	while it:
		if pred.call(it):
			it.select(0)            # TreeItem API in Godot 4
