...
var key_cache:Array[ItemDB.Keys] = []
func _ready() -> void:
	## change target enum for unlocked recipe instead of everything
	for craftable in ItemDB.CRAFTABLES:
		var craftable_item:CraftableResource = ItemDBUtility.get_item_resource(ItemDB.CRAFTABLES[craftable])
...
func _on_craftable_selected(index:int):
	...
	current_craft = ItemDB.CRAFTABLES[key_cache[index]]
	...
	for item_key in craftable_item.required_material.keys():
		var required_amt = craftable_item.required_material[item_key]
		var resource = ItemDBUtility.get_item_resource(item_key)
...
func craft_item(target_item: ItemDB.Keys) -> bool:
	...
	for item_key in craftable_item.required_material.keys():
		...
		for slot in player_inventory:
			if slot.item_key == item_key:
				owned_count += slot.count
		if owned_count < required_amt:
			print("Not enough %s to craft %s" % [ItemDBUtility.get_item_name(item_key), ItemDBUtility.get_item_name(target_item)])
			return false  # Insufficient materials

	# Remove required materials
	for item_key in craftable_item.required_material.keys():
		var required_amt = craftable_item.required_material[item_key]
		var removed = InventoryManager.remove_item_from_inventory(player_inventory,item_key, required_amt)
		if removed < required_amt:
			push_error("Material removal failed for %s" % ItemDBUtility.get_item_name(item_key))
			return false
